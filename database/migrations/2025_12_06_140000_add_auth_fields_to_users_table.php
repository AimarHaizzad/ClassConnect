...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Add unique constraint to user_id if column exists but constraint doesn't
        if (Schema::hasColumn('users', 'user_id')) {
            $indexes = \Illuminate\Support\Facades\DB::select("SHOW INDEXES FROM users WHERE Column_name = 'user_id' AND Non_unique = 0");
            if (empty($indexes)) {
                \Illuminate\Support\Facades\DB::statement('ALTER TABLE users ADD UNIQUE KEY users_user_id_unique (user_id)');
            }
        }
    }
